...
const { state, employeeById, formatDate, submitLeaveRequestToBackend, updateLeaveStatusOnBackend } = useHrState()
...
async function submitLeaveRequest() {
  ...
  try {
    await submitLeaveRequestToBackend({
...
async function updateLeaveStatus(requestId, newStatus) {
  try {
    await updateLeaveStatusOnBackend(requestId, newStatus)
  } catch (err) {
    submitError.value = err.message
  }
}
