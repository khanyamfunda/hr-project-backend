...
const dashboardStats = computed(() => {
  ...
  const monthlyPayroll = state.employees.reduce((sum, employee) => {
    const payrollRecord = state.payrollSource.find((entry) => entry.employeeId === employee.id)
    return sum + (payrollRecord ? Number(payrollRecord.finalSalary) : Number(employee.salary) / 12)
  }, 0)
  const averageRating =
