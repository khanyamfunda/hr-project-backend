...
router.post('/employee-login', async (req, res) => {
    try {
        ...
        const [users] = await pool.query(`
            SELECT u.user_id, u.employee_id, u.username, u.role, e.email
            FROM users u
            JOIN employees e ON e.employee_id = u.employee_id
            WHERE u.employee_id = ? AND LOWER(e.email) = ? AND u.is_active = TRUE
        `, [employeeId, email]);

        if (!users.length) {
            return res.status(401).json({ error: 'Employee email does not match the selected profile.' });
        }
        ...
        res.status(500).json({ error: 'Employee login failure: ' + err.message });
    }
});
