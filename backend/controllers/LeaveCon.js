...
// @desc    Get leave requests — the full HR queue for HR/Manager, or just
//          the logged-in employee's own requests for everyone else
...
export const getAllLeaveRequests = async (req, res) => {
    try {
        ...
        if (isPrivileged) {
            const [rows] = await pool.query(`${baseQuery} ORDER BY lr.created_at DESC`);
            return res.status(200).json(rows);
        }

        const employeeId = req.user.employee_id ?? req.user.id;
        const [rows] = await pool.query(
            `${baseQuery} WHERE lr.employee_id = ? ORDER BY lr.created_at DESC`,
            [employeeId]
        );
        return res.status(200).json(rows);
    } catch (error) {
